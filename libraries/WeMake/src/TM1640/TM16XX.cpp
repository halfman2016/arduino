...
void TM16XX::setDisplayToStringEx(const char* string,const byte dotsPos, const byte pos,const byte font[])
{
	for (int i = 0; i < displays - pos; i++) {
  	if (string[i] != '\0') 
	{
	  if(i < dotsPos)
	  {
	  	sendChar(i + pos, font[string[i] - 32],i == dotsPos-1);
	  }
	  else if(i == dotsPos)
	  {
	  ...
	  {
	  	sendChar(i + pos -1, font[string[i] - 32],0);
	  }
	}
	else 
	{
