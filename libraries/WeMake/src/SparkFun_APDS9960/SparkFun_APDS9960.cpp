...
uint8_t SparkFun_APDS9960::getCurrentColor()
{
	uint16_t ambient_light = 0;
	uint16_t r = 0;
	uint16_t g = 0;
	uint16_t b = 0;
	uint8_t color = 1;// 1��ɫ 2��ɫ 3��ɫ 4 ��ɫ 5��ɫ 6δʶ��
	uint8_t min = 1;//������СֵΪ��ɫ
	if (!this->readAmbientLight(ambient_light) ||
        !this->readRedLight(r) ||
        !this->readGreenLight(g) ||
        !this->readBlueLight(b) ) 
   	{
		...
		else if (color == 2 && g > 260) {
		}
		else if (color == 1 && r > 200) {
		}
		else
        {
            return 6;//��ɫʶ��ʧ��
